/**
 * SkillDetail Page — Full skill browser view.
 */

import { useParams, Link } from 'react-router-dom';
import { ArrowLeft, Wrench, Clock, Tag } from 'lucide-react';
  ...
  if (error || !resource) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-6">
        <Link to="/skills" className="flex items-center gap-1 text-xs mb-4 hover:text-honey-500 transition-colors" style={{ color: 'var(--color-text-muted)' }}>
          <ArrowLeft className="w-3.5 h-3.5" />
          Back to Skills
        </Link>
        <div className="card p-8 text-center">
          <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>Skill library not found</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-4">
      <div>
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: 'var(--color-elevated)' }}>
