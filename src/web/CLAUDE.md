...
## Spec discussion (Discussion tab)

`SpecDetail` carries a two-tab strip under the header — **Document** (the Tiptap editor) and **Discussion** — deep-linkable via `?tab=discussion` (system turns / toasts jump straight in). The right-hand dispatch panel stays visible on both tabs.

- **`SpecDiscussionPanel`** (`components/specs/SpecDiscussionPanel.tsx`) resolves the spec's mail conversation via `useSpecThread` (`GET /specs/:rid/:sid/thread`, which returns `null` on 404 — no error). No thread → "Start discussion" CTA that POSTs `useCreateSpecThread` and swaps in the view. Thread exists → mounts `MailThreadView` as-is (realtime, roster, closed-state gating all reused).
- **`InviteAgentButton`** (`components/sessions/InviteAgentButton.tsx`) is passed to `MailThreadView` via the new optional `headerAction` prop. It lists online **mail-capable** agents (any agent on a `mail.canJoin` swarm — not gated on ACP) grouped per swarm, filtering sidecars + existing participants, and POSTs `/mail/conversations/:id/participants` (`useInviteMailParticipant`). Roster refreshes from the conversation-query invalidation + the live `mail.participant.joined` event (now fanned out to `mail:conversation:<id>`, not just the list channel). The popover states the async delivery caveat (invited agents reply on next activation / dispatch).
- **System turns** — hub-authored `system:dispatch-orchestrator` outcome turns (dispatch terminal states, see `src/specs/spec-conversation.ts` `postSpecThreadOutcome`) render in `MailThreadView` relabeled as "Orchestrator" (detected by `sender.startsWith('system:')`) instead of an agent literally named `system:dispatch-orchestrator`.
- **Threads list** — `Sessions.tsx` `mailToThread` special-cases the `spec-thread` scope to link at `/specs/:rid/:sid?tab=discussion` (via `metadata.spec_resource_id` + `spec_id`), so the spec page and the generic mail view don't compete.
