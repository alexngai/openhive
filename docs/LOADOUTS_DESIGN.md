...
- **Dispatch enrichment**: `enrichWithLoadout` resolves a binding in precedence order — the **dispatch row's `loadout_resource_id` / `team_template_resource_id` + `role`** (V64, set by the dispatch modal via `readRowLoadoutBinding`) first, then the spec node's `spec_metadata.loadout_ref` (preferred) / `team_role_ref`. It materializes live from resource ids, attaches as `task.metadata.materializedLoadout`, **and registers the materialization in the loadout side-channel keyed by taskId**. A bare row `role` (no loadout to materialize) still surfaces onto `task.metadata.role` so `chooseExecutor` filters the roster. Failures attach a `loadout_error` marker and broadcast a `dispatch.materialization_failed` event on `map:dispatches` rather than blocking dispatch. (Note: the row's pinned `loadout_bundle_id` / `team_bundle_id` are a *separate* content-addressed surface consumed only by the sidecar-facing `GET /dispatches/:id/loadout` via `src/openteams/loadout-materializer.ts`.)
