...
> **Follow-up wave — closed (2026-07-03).** The four in-repo deferrals from the P4/P5 waves shipped:
> - **P4.3 linked-task rollup** — `DispatchRollup` now takes an optional `tasks` prop and appends a `N/M tasks` chip (`rollupTaskStatuses`, done-statuses = closed/done/complete/completed/resolved). Plumbed from `data.linked.tasks` through `SpecDetail` (header + `SpecDispatchPanel`). Dispatch status stays the primary signal.
> - **P4.4 MailThreadView swap** — `DispatchThreadSection` now delegates to the unified `MailThreadView` (realtime WS, shared avatars, `system:`→"Orchestrator" relabel) whenever a `conversation_id` exists; the lazy-create empty-state composer (turns route) is kept only for the no-conversation-yet active case. The two blockers were resolved: `usePageContext` gained an `{ enabled }` option so the embedded view (`registerPageContext={false}`) no longer stomps the dispatch page's context; and the mail send path is gated only on `conversation.status === 'active'` (not agent presence), preserving fire-and-forget semantics.
> - **P5.2 inline diff** — the outcome view now embeds `StackDiffView mode="stream"` behind a per-artifact *Show/Hide diff* toggle (lazy-mounted), keeping the *Open in Changes →* deep link and the labeled "diff not indexed" fallback.
> - **Daemon restart resilience** — `ensureDaemon` now reaps a stale opentasks lock (proper-lockfile's lingering `daemon.lock.lock` dir + the dead-pid `daemon.lock` content file) before spawning, once the socket is confirmed dead — eliminating the ~10s `ELOCKED`/500 transient after an ungraceful container restart. See `reapStaleDaemonLock`.
>
> **Still cross-repo (blocked on a sidecar consumer):** **P4.5 Layers 2 & 3** — proactive cancel for `origin === 'mail'` dispatches. The hub-side transport can already emit `x-dispatch/cancel` (mail + MAP scopes, tested), and the `cancel_not_acked` ribbon renders, but real/immediate cancel + the `dispatch.cancel_acknowledged` success ribbon need a cooperative stop+ack consumer in **macro-agent / claude-code-swarm**. No further in-repo work lands value until that ships.
