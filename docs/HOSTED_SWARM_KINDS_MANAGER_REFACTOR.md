...
2. **The provider interface is mostly fine** — `provision(config) → ProvisionResult` doesn't need to change for claude-code. What needs to change is *what config we pass* and *what we wait for after*. Both can be done by branching at the manager level without touching the provider contract.

Branching early in `spawn()` means: claude-code gets its own ~150-line method (`spawnClaudeCode`) that does only what claude-code needs. The two methods share helpers (`allocatePorts`, `releasePorts`, DB writes) but the orchestration is per-kind. Duplication is real but bounded — ~150 lines of overlap with the existing path.
...
Phase 11 Provider.provision() — pass `claude` command, no port       [shared, kind-aware command]
Phase 12 Wait for cc-swarm sidecar MAP registration (not HTTP health) [claude-code-specific]
Phase 13 Mark running, broadcast event                               [shared]
```

**4. Provider command resolution.** The local provider currently builds the command from `this.config.openswarm_command`. For claude-code, it needs to use `claude` instead. Options:

- **(preferred)** Pass the resolved command into `SwarmProvisionConfig` as a new optional field; provider uses it if present, falls back to `openswarm_command` otherwise. Lets the manager (or its kind branch) decide the command without provider needing kind awareness.
- Alternatives (rejected): kind-aware provider instance per kind, env-var-based command override. Both spread kind awareness wider than needed.
...
interface SwarmProvisionConfig {
   ...
   * it's used verbatim. Used by claude-code to spawn the `claude` binary.
   */
...
Local provider reads these in its existing command-build path; one branch.
...
Each of these is a "works in approach B" item. They're not blockers for the spike; they're scope creep.
...
| Workspace repos clone in provider | Should work as-is; verify in spike |
...
| `getInstanceId()` returns null silently | Same behavior; not a regression |
| Restart HOT vs COLD | Always cold for claude-code (step 9) |
...
| `restartCounts` keyed by hosted_swarm_id | Survives restarts cleanly; works for both kinds |
...
4. **PR 4**: implement `spawnClaudeCode()` happy path. Uses PR 3's provider override. Mocks the cc-swarm sidecar registration wait initially (just resolves immediately if MAP registration arrives within timeout, else fails).
